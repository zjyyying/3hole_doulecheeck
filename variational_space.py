...
def exist_d6_d7_state(o1,o2,o3,z1,z2,z3):
                                ...
                                for tx in range(-Mc,Mc+1):
                                    ...
                                    for ty in range(-Bt,Bt+1):
                                        for tz in [0]:
                                            ...
                                            if not check_in_vs_condition1(ux,uy,vx,vy,tx,ty):
                                                continue

                                            #the function is used to decrease the for circulation
                                            funlist = [util.lamlist(orb1s, orb2s, orb3s)]
                                            for f1 in funlist[0]:
                                                orb1, orb2, orb3 = f1()
...
#                                                        continue
    ...
    def get_state(self,uid):
        ...
        uid_ = uid_ % (64*N3*B4) 
        x2 = int(uid_/(64*N3*B3))- s 
        uid_ = uid_ % (64*N3*B3)
        y2 = int(uid_/(64*N3*B2))- s  
        uid_ = uid_ % (64*N3*B2)
        x1 = int(uid_/(64*N3*B1))- s 
        uid_ = uid_ % (64*N3*B1)
        y1 = int(uid_/(64*N3)) - s
        uid_ = uid_ % (64*N3)       
        o3 = int(uid_/(64*N2))
